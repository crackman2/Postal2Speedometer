...
begin
 ...
 { MessageBox(0,PChar('VelX: ' + FloatToStr(vel.x) + sLineBreak +
                     'VelY: ' + FloatToStr(vel.y) + sLineBreak +
                     'dirAngleX: ' + FloatToStr(dirAngleX) + sLineBreak +
                     'dirAngleY: ' + FloatToStr(dirAngleY) + sLineBreak +
                     'velocityMagnitude: ' + FloatToStr(velocityMagnitude) + sLineBreak
  ),'a',0);  }
  ...
  begin
   ...
   { MessageBox(0,PChar('vel.x       : ' + FloatToStr(vel.x) + sLineBreak +
                       'vel.y       : ' + FloatToStr(vel.y) + sLineBreak +
                       'normalizedVX: ' + FloatToStr(normalizedVX) + sLineBreak +
                       'normalizedVY: ' + FloatToStr(normalizedVX) + sLineBreak
    ),'a',0);   }
   ...
   { MessageBox(0,PChar('dotProduct: ' + FloatToStr(dotProduct) + sLineBreak +
                       'new vel.x : ' + FloatToStr(vel.x) + sLineBreak +
                       'new vel.y : ' + FloatToStr(vel.y) + sLineBreak
    ...
    begin
     { MessageBox(0,PChar('velocity to large at: ' + FloatToStr(velocityMagnitude)),'a',0);
      normalizedVX := vel.x / velocityMagnitude;
      normalizedVY := vel.y / velocityMagnitude;

      MessageBox(0,PChar('normalx: ' + FloatToStr(normalizedVX) + sLineBreak +
                         'normaly: ' + FloatToStr(normalizedVY)
      ),'a',0); }
      ...
      {MessageBox(0,PChar('velx: ' + FloatToStr(vel.x) + sLineBreak +
                       'vely: ' + FloatToStr(vel.y)
       ),'a',0);}

      velocityMagnitude := Sqrt((vel.x * vel.x) + (vel.y * vel.y));
      { MessageBox(0,PChar('new velocity is: ' + FloatToStr(velocityMagnitude)),'a',0);  }
      //if velocityMagnitude > maxspeed then MessageBox(0,PChar('crap:' + floattostr(vel.x)),'fuck',0);
    end;
  end;
end;
